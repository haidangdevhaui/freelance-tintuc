...
<div class="menu-drop">
	<nav class="navbar">
		<ul class="navbar-nav">
			...
			<li>
				<a {{ $post_detail->category_id == $sub->id ? 'style=color:red' :''}} href="{{route('desktop.category', ['slug' => $sub->slug])}}">
					{{$sub->name}}
				</a>
			</li>
			@endforeach
		...
		<div class="primary">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">
					<a href="{{route('desktop.category', ['slug' => $link[0]->slug])}}">
						{{$link[0]->name}}
					</a>
				</li>
				@if(isset($link[1]))
				<li class="breadcrumb-item active">
					<a href="{{route('desktop.category', ['slug' => $link[1]->slug])}}">
						{{$link[1]->name}}
					</a>
				</li>
				@endif
